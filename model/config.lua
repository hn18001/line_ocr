function getConfig()
    local config = {
        ...
        optimConfig      = {},
        trainSetPath     = '/data/heneng/github/crnn/data/3567_1m/train5m_4color_7font_shadow_lmdb/data.mdb',
        valSetPath       = '/data/heneng/github/crnn/data/3567_1m/val5m_4color_7font_shadow_lmdb/data.mdb',
    }
    return config
end
...
function createModel(config)
    ...
    model:add(cudnn.SpatialMaxPooling(2, 2, 2, 2))       -- 64x16x50  240
    model:add(convRelu(2))
    model:add(cudnn.SpatialMaxPooling(2, 2, 2, 2))       -- 128x8x25  120
    model:add(convRelu(3, true))
    model:add(convRelu(4))
    --model:add(cudnn.SpatialMaxPooling(2, 2, 1, 2, 1, 0)) -- 256x4x26  
    model:add(cudnn.SpatialMaxPooling(2, 2, 2, 2, 0, 0)) -- 256x4x26  60
    model:add(convRelu(5, true))
    model:add(convRelu(6))
    --model:add(cudnn.SpatialMaxPooling(2, 2, 1, 2, 1, 0)) -- 512x2x27
    model:add(cudnn.SpatialMaxPooling(2, 2, 2, 2, 0, 0)) -- 512x2x27  30
    model:add(convRelu(7, true))                         -- 512x1x26  29
    model:add(nn.View(512, -1):setNumInputDims(3))       -- 512x26    29
    model:add(nn.Transpose({2, 3}))                      -- 26x512    29x512
